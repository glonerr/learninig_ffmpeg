...
font_t* bbutil_load_font(const char* path, int point_size, int dpi)
{
    ...
    GLubyte* font_texture_data = (GLubyte*) calloc(2 * font_tex_width * font_tex_height,
            sizeof(GLubyte));
    ...
    for (c = 0; c < 128; c++) {
        ...
        for (j = 0; j < glyph_height; j++) {
            for (i = 0; i < glyph_width; i++) {
                font_texture_data[2
                        * ((bitmap_offset_x + i) + (j + bitmap_offset_y) * font_tex_width) + 0] =
                        font_texture_data[2
                                * ((bitmap_offset_x + i) + (j + bitmap_offset_y) * font_tex_width)
                                + 1] =
        ...
        font->offset_y[c] =
                (float) ((slot->metrics.horiBearingY - face->glyph->metrics.height) >> 6);
    }
...
void bbutil_render_text(font_t* font, const char* msg, float x, float y, float r, float g, float b,
        float a)
{
    int i, c;
    GLfloat *vertices;
    ...
    if (!text_program_initialized) {
        ...
        const char* v_source = "precision mediump float;"
        ...
        const char* f_source = "precision lowp float;"
                "varying vec2 v_texcoord;"
...
#ifdef USING_GL20
void bbutil_render_text_angle(font_t* font, const char* msg, float x, float y, float r, float g,
        float b, float a, float angle)
{
    int i, c;
    GLfloat *vertices;
    GLfloat *texture_coords;
    ...
    for (i = 0; i < msg_len; ++i) {
        ...
        vertices[8 * i + 0] = x + (pen_x + font->offset_x[c]) * cos(angle);
        vertices[8 * i + 1] = y - (pen_x + font->offset_y[c]) * sin(angle);
        vertices[8 * i + 2] = vertices[8 * i + 0] + font->width[c] * cos(angle);
        vertices[8 * i + 3] = vertices[8 * i + 1] - font->width[c] * sin(angle);
        vertices[8 * i + 4] = vertices[8 * i + 0] + font->height[c] * sin(angle);
        vertices[8 * i + 5] = vertices[8 * i + 1] + font->height[c] * cos(angle);
        vertices[8 * i + 6] = vertices[8 * i + 2] + font->height[c] * sin(angle);
        vertices[8 * i + 7] = vertices[8 * i + 3] + font->height[c] * cos(angle);
...
int bbutil_load_texture(const char* filename, int* width, int* height, float* tex_x, float* tex_y,
        unsigned int *tex)
{
    int i;
    GLuint format;
    ...
    // get info about png
    png_get_IHDR(png_ptr, info_ptr, &image_width, &image_height, &bit_depth, &color_type, NULL,
            NULL, NULL);
    ...
    switch (color_type) {
        ...
        case PNG_COLOR_TYPE_RGB:
            ...
            break;
        default:
            fprintf(stderr, "Unsupported PNG color type (%d) for texture: %s", (int) color_type,
                    filename);
            fclose(fp);
            png_destroy_read_struct(&png_ptr, &info_ptr, &end_info);
            return NULL;
    ...
    if ((tex_width != image_width) || (tex_height != image_height)) {
        glTexImage2D(GL_TEXTURE_2D, 0, format, tex_width, tex_height, 0, format, GL_UNSIGNED_BYTE,
                NULL);
        glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, image_width, image_height, format, GL_UNSIGNED_BYTE,
                image_data);
    } else {
        glTexImage2D(GL_TEXTURE_2D, 0, format, tex_width, tex_height, 0, format, GL_UNSIGNED_BYTE,
                image_data);
...
int bbutil_calculate_dpi(screen_context_t ctx)
{
    ...
    rc = screen_get_display_property_iv(screen_disp, SCREEN_PROPERTY_PHYSICAL_SIZE,
            screen_phys_size);
    if (rc) {
